...
ifeq ($(config),debug)
  RESCOMP = windres
  TARGETDIR = bin/Debug
  TARGET = $(TARGETDIR)/premake-raylib-starter-x86_64-debug
  OBJDIR = obj/Debug
  DEFINES += -DDEBUG
  INCLUDES +=
  FORCE_INCLUDE +=
...
ifeq ($(config),release)
  ...
  TARGET = $(TARGETDIR)/premake-raylib-starter-x86_64-release
  OBJDIR = obj/Release
  DEFINES += -DNDEBUG
  INCLUDES +=
  FORCE_INCLUDE +=
...
$(TARGET): $(GCH) ${CUSTOMFILES} $(OBJECTS) $(LDDEPS) $(RESOURCES) | $(TARGETDIR)
	@echo Linking prj_raylib_starter
	$(SILENT) $(LINKCMD)
	$(POSTBUILDCMDS)
...
clean:
	@echo Cleaning prj_raylib_starter
ifeq (posix,$(SHELLTYPE))
	$(SILENT) rm -f  $(TARGET)
	$(SILENT) rm -rf $(OBJDIR)
else
